...
#pragma pack(1)
	...
	typedef struct
	{
		uint8_t mode;                      // motor working mode 电机工作模式
		float q;                           // current angle (unit: radian) 当前的角度
		float dq;                          // current velocity (unit: radian/second) 当前的角速度
		float ddq;                         // current acc (unit: radian/second*second) 当前的角加速度
		float tauEst;                      // current estimated output torque (unit: N.m) 电流估计输出转矩(单位:N.m)
